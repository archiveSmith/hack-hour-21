...
function bestProfit(stock_prices_yesterday) {
  ...
  for (let buy=0; buy<stock_prices_yesterday.length; buy++){ // loop through each price
    ...
    for(let sell=buy+1; sell<stock_prices_yesterday.length; sell++){ // loop through future prices
      currSell = stock_prices_yesterday[sell];
      if (currSell < 0) return 0;
      if ( (currSell-currBuy) > currMaxProfit ) currMaxProfit = currSell - currBuy;
    }
  }
...
function bestProfitAgain(stock_prices_yesterday) {
  // check for invalid input
  if (stock_prices_yesterday === null || stock_prices_yesterday === undefined) return 0;
  ...
  let highest = [...stock_prices_yesterday].sort();
  highest.sort();
  
  console.log(highest);

  let lowestIdx = 0; // for walking the array of lowest prices
  let highestIdx = highest.length-1; // for walking the array of the highest prices
